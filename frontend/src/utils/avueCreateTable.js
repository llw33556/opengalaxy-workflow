 ...
 * 获取form表结构
 */
import {isInArray, isRealObj} from "./index";
...
function getTableColumns1(that, respRet) {
  ...
  for (var key in respRet) {
    ...
    } else if (key == "field_custom_with") {
      fieldCustomWith = respRet[key]
    }else if (key == "field_sortable") {
      sortableList = respRet[key]
    }
  }
  ...
  if (isRealObj(searchList) && searchList.length > 0) {
      for (var q = 0; q < searchList.length; q++) {
        if(!that.listQuery.hasOwnProperty(searchList[q])&&!that.listQuery.hasOwnProperty(searchList[q]+"__in")){
          if(!isInArray(searchMList,searchList[q])){
            that.listQuery[searchList[q]] = undefined
          }else{
            that.listQuery[searchList[q]+"__in"] = undefined
          }
        }
      }
    }
  ...
  for (var key in fields) {
    ...
    //自定义宽度
    for(var fieldkey in fieldCustomWith){
      if(fieldkey==key){
        column.width=fieldCustomWith[key]
        break
      }
    }
     if (renderer.isInArray(sortableList, key)) {
