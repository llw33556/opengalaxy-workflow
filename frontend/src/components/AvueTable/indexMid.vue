<template>
  <div>
    <strong style="margin-left: 5px">{{tableTitle}}</strong>
...
<script>
  ...
  export default {
    ...
    created() {
      if (this.listQueryExtra&&this.listQueryExtra.length>0) {
        for (var i = 0; i < this.listQueryExtra.length; i++) {
          if(this.listQueryExtra[i].indexOf("=")>0){
            this.listQuery[this.listQueryExtra[i].split('=')[0]] = this.listQueryExtra[i].split('=')[1]
          }else{
    ...
    mounted() {
      if (isRealObj(this.searchMList)&&this.searchMList.length>0) {
