<template>
  <div class="app-container">
    <reviewTable ref="wait"
                 :list="workorderWaitingList"
                 :listQuery="waitingListQuery"
                 :tabletitle="waitWoTitle"
                 :total="waitingTotal"
                 ...
                 @getList="getList"
    />
    <reviewTable ref="done"
                 :list="workorderDoneList"
                 :listQuery="doneListQuery"
                 :tabletitle="doneWoTitle"
                 :total="doneTotal"
                 @handleReview="handleReview"
                 @handleDetail="handleDetail"
                 @handleFilter="handleFilter"
    ...
    <el-dialog :title="workorderName" :visible.sync="dialogFormVisible" width="85%" :close-on-click-modal='false'>
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px"
        ...
        <el-form-item :label="$t('table.review')" prop="opinion">
          <el-select v-model="temp.opinion" filterable style="width: 83%!important; margin-left: 15px"
                     @change="handleChange">
        ...
        <el-form-item :label="$t('table.nextreviewer')" prop="next_user" v-if="isShowNextUser&& isShow"
                      style="width: 85.5%!important;">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-show="dialogStatus!='detail'" type="primary" @click="reviewData()" size="mini"
                   :loading="reviewLoading">{{ $t('table.confirm')}}
        </el-button>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        reviewROptions: [],
        isShowMine: true,
        workorderWaitingList: [],
        waitingTotal: 0,
        waitWoTitle: "待审批工单",
        waitingListLoading: true,
        waitingListQuery: {
          page: 1,
          pagesize: 10,
          ordering: '-id',
    ...
    methods: {
      ...
      getWaitingList() {
        if (this.mine) {
          this.waitingListQuery.cur_user = this.$store.getters.user_id
        } else {
          this.waitingListQuery.cur_user = undefined
        }
        Workorder.list(this.waitingListQuery).then(response => {
          this.workorderWaitingList = response.results
          this.waitingTotal = response.count
        })
        setTimeout(() => {
      ...
      getDoneList() {
        Workorder.list(this.doneListQuery).then(response => {
          this.workorderDoneList = response.results
          this.doneTotal = response.count
        })
        setTimeout(() => {
