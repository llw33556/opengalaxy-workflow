<template>
  <div class="app-container">
    <reviewTable ref="history"
                 :list="workorderHistoryList"
                 ...
                 @getList="getHisList"
    />
    <el-dialog :title="workorderName" :visible.sync="dialogFormVisible" width="85%">
      ...
      <form-create ref="historyForm" v-model="workorderFields" :rule="formRule" :option="option"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getHisList() {
        Workorder.list(this.historyListQuery).then(response => {
          this.workorderHistoryList = response.results
          this.historyTotal = response.count
        })
        setTimeout(() => {
