...
from utils.util import random_char_list, send_sms, send_html_mail, Token, request_get, request_post
...
class UserViewSet(mixins.CreateModelMixin, BaseGenericViewSet):
    ...
    def logout(self, request, *args, **kwargs):
        ...
        if settings.SSO_CLIENT_ENABLE:
            redirect = settings.SSO_LOGOUT_URL
    ...
    def ssologin(self, request, *args, **kwargs):
        ...
        local_redirect_url="http://" + request.get_host() + settings.SSO_REDIRECT_LOCAL_URI
        redirect_url = settings.SSO_REDIRECT_URL+local_redirect_url
        logger.error(code)
        if not code:
            response = HttpResponseRedirect(redirect_url)
            return response

        get_token_url = settings.SSO_OAUTH_TOKEN_URL
        post_data = {
        ...
        headers = {
            ...
            "Authorization": "Basic "+settings.SSO_AUTHORIZATION,
        }
        # 获取token
        r1, err1 = request_post(get_token_url, post_data, headers=headers)
        if err1:
            return Response({"detail": err1}, status=status.HTTP_400_BAD_REQUEST)
        #获取用户信息
        rets1 = r1.json()
        get_user_info_url = settings.SSO_USER_INFO_URL + "?access_token="+rets1["access_token"]
...
class UsersViewSet(ImportMixin, ExportMixin, BaseModelViewSet):
    ...
    def get_users(self, request):
        ...
        """
        logger.error("=======================+>")
        logger.error(len(self.queryset))
        logger.error(len(User.objects.filter(is_active=True).order_by('-id')))
