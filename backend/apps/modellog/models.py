...
class LogsEntry(models.Model):
    ...
    content_type = models.ForeignKey(to=ContentType, on_delete=models.SET_NULL,
                                     verbose_name="操作对象", blank=True, null=True)
    ...
    def get_table_info(self):
        ...
        data["fields"]["model"] = "对象"
        data["field_search"].remove("user")
        data["field_show_order"] = ["id","time_added","user","action_flag","model","message"]
        data['field_select_kv'].append({"content_type": [[row.id, row.model] for row in ContentType.objects.order_by('-id')]})
        return data
